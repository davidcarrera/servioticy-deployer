...
echo Waiting for CouchBase Buckets to load...
rm -f $CB_STATUS_FILE
status=`grep -s healthy $CB_STATUS_FILE | sed 's/ //g' | tail -1`
while [ -z $status ]
do
        sleep 1
        curl -s -X GET http://$CB_MASTER:8091/pools/default | grep status > $CB_STATUS_FILE
        status=`grep -s healthy $CB_STATUS_FILE | sed 's/ //g' | tail -1`
done
rm -f $CB_STATUS_FILE
...
do
	...
	do
	        sleep 1
	        curl -s -X GET http://$CB_MASTER:8091/pools/default/buckets/$bucket | grep status > $CB_STATUS_FILE
	        status=`grep -s healthy $CB_STATUS_FILE | sed 's/ //g' | tail -1`
	done
	rm -f $CB_STATUS_FILE
